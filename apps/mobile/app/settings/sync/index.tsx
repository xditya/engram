...
function Chooser() {
  ...
  // First time on: mint the key and show the phrase once. A device linked from another already has one.
  async function turnOn(backend: SyncBackend) {
    if (!engram) return;
    const had = await engram.sync.masterKey.get();
    if (!had) await engram.sync.masterKey.ensure();
    patch('sync', { backend });
    if (had) router.back(); else router.replace('/sync/phrase');
  }
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: space[4], gap: space[3] }} keyboardShouldPersistTaps="handled">
      ...
      <Card title="iCloud Drive" body="Uses your iCloud storage. Apple only ever stores encrypted files.">
        {RN.OS === 'ios' ? (
          <>
            <Button title="Turn on" variant="outline" onPress={() => void turnOn('icloud')} />
...
const ICON = { off: 'sync-up-to-date', upToDate: 'sync-up-to-date', syncing: 'sync-syncing', unreachable: 'sync-unreachable', full: 'sync-full' } as const;
...
function Status() {
  ...
  const fix =
    status.state === 'unreachable'
      ? sync.backend === 'gdrive' ? { title: 'Sign in again', run: async () => { if (engram) { await signInGoogle(engram.platform.keys); void engram.sync.syncNow(); } } }
    ...
    : status.state === 'full'
      ...
      : { title: 'Check server', run: stop }
    : null;
  const trouble = status.state === 'unreachable' || status.state === 'full';
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: space[4], gap: space[3] }}>
      ...
      <Button title="Sync now" disabled={status.state === 'syncing'} onPress={() => void engram?.sync.syncNow().then(() => show('Synced'))} />
